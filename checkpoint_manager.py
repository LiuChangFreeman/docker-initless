...
def checkpoint_container(config,settings):
    ...
    command_create_checkpoint = "docker checkpoint create {} --checkpoint-dir={} {}".format(container_name,path_checkpoint_parent,checkpoint_name)

    path_checkpoint_current=os.path.join(path_checkpoint_parent,checkpoint_name)
    if os.path.exists(path_checkpoint_current):
        log.info("Checkpoint {} exists, remove it".format(path_checkpoint_current))
...
def main():
    ...
    for path_code_dir in os.listdir(code_dir):
        ...
        if os.path.exists(os.path.join(settings["checkpoint_dir"], config["checkpoint_name"],"config.json")):
            log.info("Sample {} checkpoint exists, skip it".format(path_code_dir))
            continue
        log.info("Start to process sample {}".format(path_code_dir))
